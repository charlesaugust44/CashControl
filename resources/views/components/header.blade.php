<header class="app-header">
    ...
    @if(isset($currentMonth) && $currentMonth)
            ...
            <div class="options-menu" id="optionsMenu" role="menu">
                <div class="options-menu__inner">
                    @if(isset($headerOptions) && count($headerOptions) > 0)
                        @foreach($headerOptions as $option)
                            ...
                            @elseif(($option['type'] ?? '') === 'link')
                                <a href="{{ $option['url'] ?? '#' }}" class="options-item{{ isset($option['variant']) && $option['variant'] === 'danger' ? ' options-item--danger' : '' }}" role="menuitem">
                                    @if(isset($option['icon']))
                                        <span class="options-item__icon">
                                            <i class="{{ $option['icon'] }}"></i>
                                        </span>
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['lang' => 'toggle']) }}" class="options-item" role="menuitem">
                        ...
                        <span class="options-item__label">{{ app()->getLocale() === 'pt_BR' ? 'EN' : 'PT' }}</span>
                    </a>
                    <button type="button" class="options-item theme-toggle" onclick="toggleTheme()" role="menuitem">
                        <span class="options-item__icon">
                            <i class="bi bi-sun-fill"></i>
                            <i class="bi bi-moon-fill"></i>
                        </span>
                        <span class="options-item__label">{{ __('ui.toggle_theme') }}</span>
                    </button>
                </div>
            </div>
        </div>
